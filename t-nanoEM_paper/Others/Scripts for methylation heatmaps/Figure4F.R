...
calculation_table <- matrix(as.numeric(table1[,paste0("R",1:4,"_abs")]),ncol=4)
colnames(calculation_table) <- paste0("R",1:4,"_abs")
...
# to extract genes that have a difference between alleles by >50% in the normal regions (R1 and R2) 
filter3 <- as.numeric(table1[,"R1_abs"]) > 50 & as.numeric(table1[,"R2_abs"]) > 50

# create a table
table2 <- table[filter1&filter2&filter3,paste0(sort(rep(paste0("R",1:4),2)),"_HP",1:2)]
...
par(mar=c(5, 4, 4, 8))
pdf("./output/Figure4F.pdf")
heatmap.2(scale(heatmap),Rowv=TRUE,Colv=FALSE,dendrogram="row",RowSideColors=inprinting,trace="none",col="bluered",
          density.info="none",cexRow=0.75,cexCol=1,margins=c(5,6),hclustfun=function(x){hclust(x,method="ward.D2")})
dev.off()
